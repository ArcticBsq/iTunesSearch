...
protocol SongsPresenterProtocol: AnyObject {
    init(view: SongsViewProtocol, networkService: NetworkServiceProtocol, url: String?, dataService: DataServiceProtocol, fullUrl: String?)
...
final class SongsPresenter: SongsPresenterProtocol {
    ...
    init(view: SongsViewProtocol, networkService: NetworkServiceProtocol, url: String?, dataService: DataServiceProtocol, fullUrl: String?) {
        ...
        if let _url = self.url {
            self.getSongs(url: Url.song, term: _url)
        }
